...
public final class VersionCheck
{
    ...
    public static synchronized CompletableFuture<Result> start(String remoteUrl, String configDownloadUrl, String currentVersion, int timeoutSeconds)
    {
        if (future != null)
        {
        ...
        try {
            ...
            if (sch == null || !(sch.equalsIgnoreCase("http") || sch.equalsIgnoreCase("https")))
            {
                ...
                future = CompletableFuture.completedFuture(new Result(State.ERROR, currentVersion, null, effectiveDownload, configDownloadUrl));
                return future;
            }
        }
        catch (Exception ex)
        ...
        future = client.sendAsync(request, HttpResponse.BodyHandlers.ofString()).thenApply(resp ->
                {
